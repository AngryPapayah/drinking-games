    ...
    <div class="max-w-2xl mx-auto py-10">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <form method="POST" action="{{ route('games.update', $game->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        >
                            ...
                            @foreach($gameTypes as $type)
                                <option
                                    value="{{ $type->id }}"
                                    {{ old('game_type_id', $game->game_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-end space-x-2">
                        <a href="{{ route('dashboard') }}"
                           class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                            Cancel
                        </a>
